...
import { SocketEvents, type EventNames, type EventPayloads, type RegisterSocketEventHandler, type SocketEventHandler, type SocketEventHandlerMap, type SocketServerInitProps } from '../../types/socketEventTypes.js';
...
export function createSocketServer({
    ...
    io.on("connection", (socket: Socket) => {
        //#FIXME Добавить авторизацию по токену
        Logger.log({
        ...
        for (const [event, handler] of Object.entries(mergedHandlers) as [EventNames, SocketEventHandler<any>][]) {
            if (typeof handler === "function") {
                ...
                Logger.warn({
                    message: `[Socket] The handler '${event}' must be of function type only.`,
